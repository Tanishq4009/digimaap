...
class LmoAuthService {
  ...
  Future<void> registerFingerprint(
    ...
    final response = await http.post(
      Uri.parse(
        '$baseUrl/api/v1/auth/lmo/$userId/register-fingerprint',
      ),
      headers: {
    ...
    if (response.statusCode != 200 &&
      ...
      throw ApiException(
        'Failed to register fingerprint on server',
        response.statusCode,
      );
    }
  }
  ...
  Future<bool> verifyInspectorForField(
    String inspectionId,
    String inspectorId,
  ) async {
    ...
    if (canAuthenticate) {
      try {
        didAuthenticate = await auth.authenticate(
          localizedReason:
              'Verify LMO Identity to begin field inspection',
          biometricOnly: true,
        );
      } catch (e) {
